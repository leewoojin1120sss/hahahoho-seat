(() => {
  ...
  const state = {
    ...
    lastEvent: null, eventQueue: [], particles: [], raf: null, currentColor: COLORS[0], swapCount: 0, chaosCount: 0, customBgmName: '', customBgmSrc: '', bgmMode: 'synth'
  };
  ...
  async function eventPenalty(name, seat, pool, studentPool) {
    ...
    const arena=document.createElement('div');arena.className='penalty-arena pk-v10';arena.innerHTML=`<div class="pk10-scoreboard"><section class="pk10-team left"><strong>${escapeHtml(leftName)}</strong><b class="pk10-score-num">0</b><div class="pk10-shots"></div></section><div class="pk10-center"><strong>SHOOTOUT</strong><span class="pk10-round">KICK 1 / 5</span></div><section class="pk10-team right"><strong>${escapeHtml(rightName)}</strong><b class="pk10-score-num">0</b><div class="pk10-shots"></div></section></div><div class="penalty-goal"><div class="penalty-net"></div><div class="penalty-keeper-v5"></div></div><div class="penalty-kicker"><span class="shooter-figure"></span><b></b></div><div class="penalty-ball-v5">●</div><div class="penalty-call">READY</div>`;els.penaltyField.appendChild(arena);els.penaltyField.classList.add('active-layer');audio.warning();await wait(1000);
    const scoreNums=[...arena.querySelectorAll('.pk10-score-num')],shots=[...arena.querySelectorAll('.pk10-shots')],roundLabel=arena.querySelector('.pk10-round'),kicker=arena.querySelector('.penalty-kicker'),kickerName=kicker.querySelector('b'),ball=arena.querySelector('.penalty-ball-v5'),keeper=arena.querySelector('.penalty-keeper-v5'),call=arena.querySelector('.penalty-call');
    ...
    const token=o=>`<span class="pk10-shot ${o||''}">${o==='goal'?'●':o==='save'?'◆':o==='miss'?'×':''}</span>`;
  ...
  async function seatTakeoverEvent(revealed) {
    const locked=new Set(state.locks.values()),candidates=[...revealed.entries()].filter(([s])=>!locked.has(s));if(candidates.length<2)return false;
    const [[attackerSeat,attacker],[defenderSeat,defender]]=shuffle(candidates).slice(0,2);const duel=duelOrder(attacker,defender),mode=pick(['BATTLE','RPS','DICE']);clearTransient();setEventColor('#ff8a34');hideEventLayer();els.swapField.classList.add('active-layer');
    const board=document.createElement('div');board.className='takeover-board';board.innerHTML=`<div class="takeover-title">SEAT TAKEOVER</div><div class="takeover-target">TARGET · ${seatMini(defenderSeat)}</div><div class="takeover-versus"><section class="${duel.heroLeft?'challenger':''}"><small>${duel.heroLeft?'CHALLENGER':'DEFENDER'}</small><strong>${escapeHtml(duel.leftName)}</strong><span>${duel.heroLeft?seatMini(attackerSeat):seatMini(defenderSeat)}</span></section><b>VS</b><section class="${duel.heroLeft?'':'challenger'}"><small>${duel.heroLeft?'DEFENDER':'CHALLENGER'}</small><strong>${escapeHtml(duel.rightName)}</strong><span>${duel.heroLeft?seatMini(defenderSeat):seatMini(attackerSeat)}</span></section></div><div class="takeover-game">${mode}</div>`;els.swapField.appendChild(board);audio.warning();fireworks(4,'#ff8a34');await wait(1300);
    const game=board.querySelector('.takeover-game');if(mode==='BATTLE'){for(let i=0;i<5;i++){game.textContent=`BATTLE  ${'█'.repeat(i+1)}${'░'.repeat(4-i)}`;audio.hit();screenImpact();await wait(430);}}else if(mode==='RPS'){for(const hand of ['✊  ✊','✋  ✊','✌  ✋','✊  ✌']){game.textContent=hand;audio.tick(320,.025);await wait(430);}}else{for(let i=0;i<7;i++){game.textContent=`⚄ ${1+Math.floor(Math.random()*6)}   ·   ${1+Math.floor(Math.random()*6)} ⚂`;audio.tick(260+i*28,.025);await wait(280);}}
    game.textContent=`${attacker} WINS`;board.classList.add('takeover-won');audio.victorySting();particlesBurst(.5,.5,120,colorForName(attacker),1.3);await wait(1000);els.swapField.classList.remove('active-layer');els.swapField.innerHTML='';
    const ok=await animateChaosExchange(revealed,attackerSeat,defenderSeat,'TAKEOVER');if(ok){els.swapField.classList.add('active-layer');const done=document.createElement('div');done.className='chaos-result';done.innerHTML=`<strong>SEAT STOLEN</strong><span>${escapeHtml(attacker)} → ${seatMini(defenderSeat)}</span><small>${escapeHtml(defender)} → ${seatMini(attackerSeat)}</small>`;els.swapField.appendChild(done);await wait(1200);els.swapField.classList.remove('active-layer');els.swapField.innerHTML='';state.swapCount++;state.chaosCount++;}return ok;
  }

  async function tripleRotationEvent(revealed) {
    const locked=new Set(state.locks.values()),candidates=shuffle([...revealed.entries()].filter(([s])=>!locked.has(s)));if(candidates.length<3)return false;const picks=candidates.slice(0,3);clearTransient();setEventColor('#8b5cff');hideEventLayer();els.swapField.classList.add('active-layer');const board=document.createElement('div');board.className='triple-rotate-board';board.innerHTML=`<div class="takeover-title">TRIPLE ROTATION</div><div class="triple-list">${picks.map(([s,n],i)=>`<section><small>${seatMini(s)}</small><strong>${escapeHtml(n)}</strong><b>${i<2?'→':'↻'}</b></section>`).join('')}</div>`;els.swapField.appendChild(board);audio.warning();fireworks(5,'#8b5cff');await wait(1600);els.swapField.classList.remove('active-layer');els.swapField.innerHTML='';
    const [a,b,c]=picks,pa=getSeatPoint(a[0]),pb=getSeatPoint(b[0]),pc=getSeatPoint(c[0]);if(!pa||!pb||!pc)return false;const data=[[a,pa,pb],[b,pb,pc],[c,pc,pa]],ghost=[];for(const [[seat,name],from,to] of data){const g=document.createElement('div');g.className='swap-ghost chaos-ghost';g.textContent=name;g.style.left=`${from.x}px`;g.style.top=`${from.y}px`;g.style.setProperty('--ghost-color',colorForName(name));els.cinemaStage.appendChild(g);ghost.push([g,from,to]);}els.cinemaSeats.classList.add('swap-dim');audio.whoosh();await Promise.allSettled(ghost.map(([g,from,to],i)=>g.animate([{transform:'translate(-50%,-50%)'},{transform:`translate(calc(-50% + ${(to.x-from.x)*.5}px),calc(-50% + ${(to.y-from.y)*.5-110}px)) scale(1.25) rotate(${120*(i+1)}deg)`},{transform:`translate(calc(-50% + ${to.x-from.x}px),calc(-50% + ${to.y-from.y}px))`}],{duration:2100,easing:'cubic-bezier(.2,.75,.18,1)',fill:'forwards'}).finished));ghost.forEach(([g])=>g.remove());revealed.set(b[0],a[1]);revealed.set(c[0],b[1]);revealed.set(a[0],c[1]);state.assignment.set(b[0],a[1]);state.assignment.set(c[0],b[1]);state.assignment.set(a[0],c[1]);els.cinemaSeats.classList.remove('swap-dim');renderCinemaSeats(revealed);audio.impact(true);screenImpact();fireworks(8,'#8b5cff');await wait(950);state.swapCount++;state.chaosCount++;return true;
  }

  async function mysterySwitchEvent(revealed) {
    const locked=new Set(state.locks.values()),candidates=shuffle([...revealed.entries()].filter(([s])=>!locked.has(s)));if(candidates.length<2)return false;const [a,b]=candidates.slice(0,2);clearTransient();setEventColor('#36e7ff');setEvent('MYSTERY SWITCH','???',`${seatMini(a[0])}  //  ${seatMini(b[0])}`);audio.glitch();for(let i=0;i<8;i++){els.eventTitle.textContent=i%2?'???':'SWITCH';await wait(130);}hideEventLayer();const ok=await animateChaosExchange(revealed,a[0],b[0],'MYSTERY');if(ok){fireworks(6,'#36e7ff');state.swapCount++;state.chaosCount++;}return ok;
  }

  async function runChaosMutation(revealed) {
    const roll=Math.random();if(roll<.52)return seatTakeoverEvent(revealed);if(roll<.78)return tripleRotationEvent(revealed);if(roll<.92){const ok=await swapAssignedSeats(revealed);if(ok)state.chaosCount++;return ok;}return mysterySwitchEvent(revealed);
  }
  ...
  async function runDraw() {
    ...
    while(queue.length){
      ...
      await drawEvent(ev,name,seat,available,studentPool,remaining,total-queue.length-1,total);await commitAssignment(revealed,seat,name,remaining<=3,ev);assignedUnlocked++;if(remaining>6&&state.chaosCount<7&&assignedUnlocked>=5&&Math.random()<.10&&[...revealed.keys()].filter(s=>!lockedSeats.has(s)).length>=3){await runChaosMutation(revealed);}await wait(remaining<=5?820:430);
    }
    if(state.chaosCount===0&&[...revealed.keys()].filter(s=>!lockedSeats.has(s)).length>=2)await seatTakeoverEvent(revealed);
  ...
  function downloadResultImage() {
    ...
    ctx.fillStyle='#5d626e';ctx.font='700 14px system-ui, sans-serif';ctx.fillText('SEAT // CINEMATIC DRAW V10',90,1040);
  ...
  if(els.sfxVolumeInput)els.sfxVolumeInput.addEventListener('input',()=>{const v=clamp(Number(els.sfxVolumeInput.value),0,120);els.sfxVolumeValue.textContent=`${v}%`;state.sfxVolume=v/100;audio.setSfxVolume(state.sfxVolume);audio.ui(720);});
