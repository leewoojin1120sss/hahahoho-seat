(() => {
  ...
  async function seatTakeoverEvent(revealed) {
    ...
    const [[attackerSeat,attacker],[defenderSeat,defender]]=shuffle(candidates).slice(0,2),duel=duelOrder(attacker,defender),mode=pick(['BATTLE','RPS','DICE']),attackerWins=Math.random()<.52;clearTransient();setEventColor('#ff8a34');hideEventLayer();els.swapField.classList.add('active-layer');
    const board=document.createElement('div');board.className='takeover-board';board.innerHTML=`<div class="takeover-title">SEAT TAKEOVER</div><div class="takeover-target">${escapeHtml(attacker)} CHALLENGES ${escapeHtml(defender)} FOR ${seatMini(defenderSeat)}</div><div class="takeover-versus"><section class="${duel.heroLeft?'challenger':''}"><small>${duel.heroLeft?'CHALLENGER':'DEFENDER'}</small><strong>${escapeHtml(duel.leftName)}</strong><span>${duel.heroLeft?seatMini(attackerSeat):seatMini(defenderSeat)}</span></section><b>VS</b><section class="${duel.heroLeft?'':'challenger'}"><small>${duel.heroLeft?'DEFENDER':'CHALLENGER'}</small><strong>${escapeHtml(duel.rightName)}</strong><span>${duel.heroLeft?seatMini(defenderSeat):seatMini(attackerSeat)}</span></section></div><div class="takeover-game">${mode}</div>`;els.swapField.appendChild(board);audio.warning();fireworks(4,'#ff8a34');await wait(3200);
    const game=board.querySelector('.takeover-game'),showWinner=attackerWins?attacker:defender;
    if(mode==='BATTLE'){let ah=100,dh=100;for(let i=0;i<6;i++){const attackerStrikes=i%2===0,damage=randInt(9,21);if(attackerStrikes)dh=Math.max(14,dh-damage);else ah=Math.max(14,ah-damage);game.textContent=`${attacker} ${ah} HP   ⚔   ${defender} ${dh} HP`;audio.hit();screenImpact();await wait(520);}if(attackerWins)dh=0;else ah=0;game.textContent=`${attacker} ${ah} HP   ⚔   ${defender} ${dh} HP`;audio.explosion();screenImpact();await wait(620);game.textContent=`${showWinner} WINS`;}
    else if(mode==='RPS'){const rounds=generateRpsMatch();const use=attackerWins?rounds:rounds.map(r=>({hero:r.opp,opp:r.hero,heroWin:!r.heroWin}));for(let i=0;i<use.length;i++){game.textContent=`${use[i].hero}     ${use[i].opp}`;audio.tick(320+i*40,.03);await wait(720);}game.textContent=`${showWinner} WINS`;}
    else {let rolls=null;for(let tries=0;tries<80;tries++){const cand=Array.from({length:3},()=>[randInt(1,6),randInt(1,6)]),as=cand.reduce((t,x)=>t+x[0],0),ds=cand.reduce((t,x)=>t+x[1],0);if((attackerWins&&as>ds)||(!attackerWins&&ds>as)){rolls=cand;break;}}rolls=rolls|| (attackerWins?[[6,2],[5,3],[4,3]]:[[2,6],[3,5],[3,4]]);let as=0,ds=0;for(let i=0;i<rolls.length;i++){const[av,dv]=rolls[i];as+=av;ds+=dv;game.textContent=`${attacker}  ⚄ ${av} (${as})   ·   (${ds}) ${dv} ⚂  ${defender}`;audio.tick(260+i*45,.03);await wait(820);}game.textContent=`${showWinner} WINS`;}
    if(attackerWins){board.classList.add('takeover-won');audio.victorySting();particlesBurst(.5,.5,120,colorForName(attacker),1.3);await wait(1800);els.swapField.classList.remove('active-layer');els.swapField.innerHTML='';const ok=await animateChaosExchange(revealed,attackerSeat,defenderSeat,'TAKEOVER');if(ok){els.swapField.classList.add('active-layer');const done=document.createElement('div');done.className='chaos-result takeover-success';done.innerHTML=`<strong>SEAT STOLEN</strong><span>${escapeHtml(attacker)} → ${seatMini(defenderSeat)}</span><small>${escapeHtml(defender)} → ${seatMini(attackerSeat)}</small>`;els.swapField.appendChild(done);await wait(2800);els.swapField.classList.remove('active-layer');els.swapField.innerHTML='';state.swapCount++;state.chaosCount++;}return ok;}
    board.classList.add('takeover-defended');audio.confirm();particlesBurst(.5,.5,80,colorForName(defender),.9);await wait(800);board.innerHTML=`<div class="takeover-title">STEAL FAILED</div><div class="takeover-target">DEFENSE SUCCESS</div><div class="takeover-versus"><section><small>CHALLENGER</small><strong>${escapeHtml(attacker)}</strong><span>${seatMini(attackerSeat)} STAYS</span></section><b>×</b><section><small>DEFENDER</small><strong>${escapeHtml(defender)}</strong><span>${seatMini(defenderSeat)} HOLDS</span></section></div><div class="takeover-game">NO SEAT CHANGE</div>`;await wait(3000);els.swapField.classList.remove('active-layer');els.swapField.innerHTML='';state.chaosCount++;return true;
  }

  async function tripleRotationEvent(revealed) {
    const locked=new Set(state.locks.values()),candidates=shuffle([...revealed.entries()].filter(([s])=>!locked.has(s)));if(candidates.length<3)return false;const picks=candidates.slice(0,3);clearTransient();setEventColor('#8b5cff');hideEventLayer();els.swapField.classList.add('active-layer');const board=document.createElement('div');board.className='triple-rotate-board';board.innerHTML=`<div class="takeover-title">TRIPLE ROTATION</div><div class="takeover-target">THREE SEATS WILL ROTATE</div><div class="triple-list">${picks.map(([s,n],i)=>`<section><small>${seatMini(s)}</small><strong>${escapeHtml(n)}</strong><b>${i<2?'→':'↻'}</b></section>`).join('')}</div>`;els.swapField.appendChild(board);audio.warning();fireworks(5,'#8b5cff');await wait(3000);els.swapField.classList.remove('active-layer');els.swapField.innerHTML='';
    const [a,b,c]=picks,pa=getSeatPoint(a[0]),pb=getSeatPoint(b[0]),pc=getSeatPoint(c[0]);if(!pa||!pb||!pc)return false;const data=[[a,pa,pb],[b,pb,pc],[c,pc,pa]],ghost=[];for(const [[seat,nm],from,to] of data){const g=document.createElement('div');g.className='swap-ghost chaos-ghost';g.textContent=nm;g.style.left=`${from.x}px`;g.style.top=`${from.y}px`;g.style.setProperty('--ghost-color',colorForName(nm));els.cinemaStage.appendChild(g);ghost.push([g,from,to]);}els.cinemaSeats.classList.add('swap-dim');audio.whoosh();await Promise.allSettled(ghost.map(([g,from,to],i)=>g.animate([{transform:'translate(-50%,-50%)'},{transform:`translate(calc(-50% + ${(to.x-from.x)*.5}px),calc(-50% + ${(to.y-from.y)*.5-110}px)) scale(1.25) rotate(${120*(i+1)}deg)`},{transform:`translate(calc(-50% + ${to.x-from.x}px),calc(-50% + ${to.y-from.y}px))`}],{duration:2100,easing:'cubic-bezier(.2,.75,.18,1)',fill:'forwards'}).finished));ghost.forEach(([g])=>g.remove());revealed.set(b[0],a[1]);revealed.set(c[0],b[1]);revealed.set(a[0],c[1]);state.assignment.set(b[0],a[1]);state.assignment.set(c[0],b[1]);state.assignment.set(a[0],c[1]);els.cinemaSeats.classList.remove('swap-dim');renderCinemaSeats(revealed);audio.impact(true);screenImpact();fireworks(8,'#8b5cff');await wait(900);els.swapField.classList.add('active-layer');const done=document.createElement('div');done.className='chaos-result';done.innerHTML=`<strong>ROTATION COMPLETE</strong><span>${escapeHtml(a[1])} → ${seatMini(b[0])}</span><span>${escapeHtml(b[1])} → ${seatMini(c[0])}</span><small>${escapeHtml(c[1])} → ${seatMini(a[0])}</small>`;els.swapField.appendChild(done);await wait(2500);els.swapField.classList.remove('active-layer');els.swapField.innerHTML='';state.swapCount++;state.chaosCount++;return true;
  }
  ...
  async function runDraw() {
    ...
    while(queue.length){
      ...
      await drawEvent(ev,name,seat,available,studentPool,remaining,total-queue.length-1,total);await commitAssignment(revealed,seat,name,remaining<=3,ev);assignedUnlocked++;if(remaining>6&&state.chaosCount<9&&assignedUnlocked>=5&&Math.random()<.14&&[...revealed.keys()].filter(s=>!lockedSeats.has(s)).length>=3){await runChaosMutation(revealed);}await wait(remaining<=5?820:430);
    }
  ...
  function downloadResultImage() {
    ...
    ctx.fillStyle='#5d626e';ctx.font='700 14px system-ui, sans-serif';ctx.fillText('SEAT // CINEMATIC DRAW V11',90,1040);
